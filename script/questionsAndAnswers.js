const gothicTheme = {
  themeName: "Gothic",
  questionsEasy: [
    {
      id: 1,
      question: "Kto spuszcza łomot Bezimiennemu po zesłaniu do kolonii?",
      answers: [
        { text: "Bullit", correct: true },
        { text: "Diego", correct: false },
        { text: "Y'berion", correct: false },
        { text: "Wrzód", correct: false },
      ],
    },
    {
      id: 2,
      question: "Która osoba z podanych NIE jest przyjacielem Bezimiennego?",
      answers: [
        { text: "Cipher", correct: true },
        { text: "Lee", correct: false },
        { text: "Milten", correct: false },
        { text: "Diego", correct: false },
      ],
    },
    {
      id: 3,
      question:
        "Kto jest dowódcą paladynów w Górniczej Dolinie, a kto w Khorinis?",
      answers: [
        { text: "Khorinis - Lord Hagen, GD - Garond", correct: true },
        { text: "Khorinis - Lord Garond, GD - Hagen", correct: false },
        { text: "Khorinis - Lord Rhobar, GD - Orik", correct: false },
        { text: "Khorinis - Lord Orik, GD - Rhobar", correct: false },
      ],
    },
    {
      id: 4,
      question:
        "Które z podanych krain możemy odwiedzić podczas gameplayu Gothica 3",
      answers: [
        { text: "Myrtana, Nordmar, Varant", correct: true },
        { text: "Myrtana, Północne Ziemie, Varant", correct: false },
        { text: "Normard, Varant, Północne Ziemie", correct: false },
        {
          text: "Wyspy Południowe, Północne Ziemie, Niezbadane Krainy",
          correct: false,
        },
      ],
    },
    {
      id: 5,
      question: "Do oficjalnego kanonu zaliczamy, które części Gothica?",
      answers: [
        {
          text: "Gothic, Gothic 2, Gothic 2: Noc Kruka, Gothic 3",
          correct: true,
        },
        {
          text: "Gothic, Gothic 2: Noc Kruka, Gothic 3",
          correct: false,
        },
        {
          text: "Gothic, Gothic 2, Gothic 3",
          correct: false,
        },
        {
          text: "Gothic, Gothic: Mroczne Tajemnice, Gothic 2, Gothic 2: Noc Kruka, Gothic 3, Gothic 3: Zmierzch Bogów",
          correct: false,
        },
      ],
    },
    {
      id: 6,
      question: "Kto jest głównym antagonistą w dwóch pierwszych częściach gry",
      answers: [
        { text: "Śniący i Smok Ożywieniec", correct: true },
        { text: "Śniący i Bestia ", correct: false },
        { text: "Rhobar", correct: false },
        { text: "Wrzód", correct: false },
      ],
    },
    {
      id: 7,
      question: "Głównymi bossami w Gothic 2 są:",
      answers: [
        { text: "Smoki", correct: true },
        { text: "Dowódzcy wszystkich frakcji", correct: false },
        { text: "Orkowi szamani", correct: false },
        { text: "Aribbor", correct: false },
      ],
    },
    {
      id: 8,
      question: "3 główne frakcje w Gothic 1?",
      answers: [
        { text: "Stary Obóz, Nowy Obóz, Obóz na bagnie", correct: true },
        { text: "Stary Obóz, Obóz na bagnie, Obóz Quentina", correct: false },
        { text: "Nowy Obóz, Obóz Quentina, Obóz Aribbora", correct: false },
        {
          text: "Są 4 główne: Stary Obóz, Obóz Quentina, Obóz na bagnie, Nowy Obóz",
          correct: false,
        },
      ],
    },
    {
      id: 9,
      question: "W Gothicu 3, za którym Bogiem się opowiadamy?",
      answers: [
        { text: "Wybieramy nasze przeznaczenie", correct: true },
        { text: "Innosa", correct: false },
        { text: "Beliara", correct: false },
        { text: "Adanosa", correct: false },
      ],
    },
    {
      id: 10,
      question: "Jakie studio odpowiada za stworzenie Gothiców?",
      answers: [
        { text: "Piranha Bytes", correct: true },
        { text: "JoWooD", correct: false },
        { text: "CD Projekt", correct: false },
        { text: "Egmont Interactive", correct: false },
      ],
    },
  ],
  questionsMedium: [
    {
      id: 1,
      question: "Ile magów według kanonu tworzyło barierę?",
      answers: [
        { text: "12", correct: true },
        { text: "13", correct: false },
        { text: "14", correct: false },
        { text: "15", correct: false },
      ],
    },
    {
      id: 2,
      question: "Co się stanie jeśli zaprowadzimy Diega do Mora Sul?",
      answers: [
        { text: "Wykonamy nieistniejące zadanie", correct: true },
        { text: "Nic", correct: false },
        { text: "Wykonamy istniejące zadanie", correct: false },
        { text: "Ludzie z Mora Sul nas zaatakują", correct: false },
      ],
    },
    {
      id: 3,
      question: "Z kogo składa się Najwyższa Rada?",
      answers: [
        { text: "Pyrokar, Ulthar, Serpentes", correct: true },
        {
          text: "Pyrokar, Serpentes, po akcji Gothic 2 Bezimienny",
          correct: false,
        },
        { text: "Pedro, Ulthar, Rhobar", correct: false },
        { text: "Segio, Pyrokar, Rhobar", correct: false },
      ],
    },
    {
      id: 4,
      question: "Jak nazywają się dowódzcy Klanów w Nordmarze?",
      answers: [
        { text: "Grim, Tjalf, Kerth", correct: true },
        { text: "Grim, Kerth, Bogir", correct: false },
        { text: "Kerth, Tjalf, Bogir", correct: false },
        {
          text: "Tjalf, Bogir, Grim",
          correct: false,
        },
      ],
    },
    {
      id: 5,
      question: "Dokończ zdanie: 'Nie ma czego ...'?",
      answers: [
        {
          text: "plądrować",
          correct: true,
        },
        {
          text: "kraść",
          correct: false,
        },
        {
          text: "brać",
          correct: false,
        },
        {
          text: "zabijać",
          correct: false,
        },
      ],
    },
    {
      id: 6,
      question: "Yberion umiera w rozdziale",
      answers: [
        { text: "Trzecim", correct: true },
        { text: "Drugim", correct: false },
        { text: "Czwartym", correct: false },
        { text: "Kiedy zdecydujemy się go zabić", correct: false },
      ],
    },
    {
      id: 7,
      question: "Co jest główną walutą w Gothicu?",
      answers: [
        { text: "Ruda", correct: true },
        { text: "Złoto", correct: false },
        { text: "Stare monety", correct: false },
        { text: "Handel wymienny", correct: false },
      ],
    },
    {
      id: 8,
      question:
        "Który z strażników powraca i którego zabijamy w Gothic 2 Noc Kruka?",
      answers: [
        { text: "Bloodwyn", correct: true },
        { text: "Zły", correct: false },
        { text: "Thorus", correct: false },
        { text: "Fletcher", correct: false },
      ],
    },
    {
      id: 9,
      question: "Główny obozy buntowników w Myrtanie to:",
      answers: [
        { text: "Okara", correct: true },
        { text: "Reddock", correct: false },
        { text: "Obóz Anoga", correct: false },
        { text: "Nemora", correct: false },
      ],
    },
    {
      id: 10,
      question: "Kto stworzył muzykę do gier Gothic?",
      answers: [
        { text: "Kai Rosenkranz", correct: true },
        { text: "Alex Brüggemann", correct: false },
        { text: "Michael Hoge", correct: false },
        { text: "Björn Pankratz", correct: false },
      ],
    },
  ],
  questionsHard: [
    {
      id: 1,
      question: "Na której farmie mieszka Randolph i jakie zadanie/a zleca?",
      answers: [
        { text: "Farma Akila, 'Do dna' i'Brak' ", correct: true },
        { text: "Farma Akila, 'Do dna'", correct: false },
        { text: "Farma Onara, 'Brak' i 'Do dna'", correct: false },
        { text: "Farma Onara, 'Brak'", correct: false },
      ],
    },
    {
      id: 2,
      question: "Ilu jest magnatów?",
      answers: [
        { text: "Pięciu", correct: true },
        { text: "Siedmiu", correct: false },
        { text: "Trzech", correct: false },
        { text: "Dziewięciu", correct: false },
      ],
    },
    {
      id: 3,
      question: "W jakich miejscach znajdują się Artefakty Adanosa?",
      answers: [
        {
          text: "Al Shedim, Mora Sul, Gotha, Grobowiec Akaschy, Trelis",
          correct: true,
        },
        {
          text: "Grobowiec Bereka, Gotha, Klasztor magów ognia, Geldern, Ishtar",
          correct: false,
        },
        {
          text: "Grobowiec Angira, Trelis, Gotha, Mora Sul, Al Shedim",
          correct: false,
        },
        {
          text: "Grobowiec Ejnara, Gotha, Geldern, Klasztor magów ognia, Ishtar",
          correct: false,
        },
      ],
    },
    {
      id: 4,
      question: "Czym są chromaniny?",
      answers: [
        {
          text: "Księgami o magicznej aurze",
          correct: true,
        },
        { text: "Skrzyniami z artefaktami", correct: false },
        { text: "Szaty przynależności do kręgu magów onia", correct: false },
        {
          text: "Nie są z tej gry",
          correct: false,
        },
      ],
    },
    {
      id: 5,
      question:
        "W grobowcu Quarhodrona jakie są odpowiedzi na kolejne pytania?",
      answers: [
        {
          text: "Strażnicy umarłych, Kasta Wojowników, 	Kapłani, Kasta Wojowników, Uczeni, 	Uzdrowiciele, 	Skąd mam to wiedzieć?",
          correct: true,
        },
        {
          text: "Kapłani, Uczeni, Uczeni, Strażnicy umarłych, Kasta Wojowników, Uzdrowiciele, Skąd mam to wiedzieć?",
          correct: false,
        },
        {
          text: "Uczeni, Nie wiem, Strażnicy umarłych, Uzdrowiciele, Kapłani, Kasta Wojowników, Uzdrowiciele",
          correct: false,
        },
        {
          text: "Kapłani, Kasta wojowników, Uczeni, Nie wiem, Kapłani, Uzdrowiciele, Strażnicy umarłych",
          correct: false,
        },
      ],
    },
    {
      id: 6,
      question: "Ile Nordmarczyków jest w Varancie?",
      answers: [
        { text: "Sven, Orknarok, Oelk", correct: true },
        { text: "Sven, Kerth, Orknarok", correct: false },
        { text: "Tizgar, Okar, Marik", correct: false },
        { text: "Oelk, Kerth, Tizgar", correct: false },
      ],
    },
    {
      id: 7,
      question:
        "Najważniejsze pytanie. Czy Gomez wiedział o wybiciu magów ognia?",
      answers: [
        { text: "Nie", correct: true },
        { text: "Chyba nie", correct: false },
        { text: "Nie wiadomo", correct: false },
        { text: "Nah", correct: false },
      ],
    },
    {
      id: 8,
      question:
        "Gdy w Gothic 2 rozmawiamy z ostatnim bossem jako mag ognia, jakie według niego jest nasze przeznaczenie?",
      answers: [
        { text: "Głosić nauki Innosa", correct: true },
        { text: "Wypalać ogień Innosa w jego wrogach", correct: false },
        {
          text: "Szerzyć ogień Innosa w sercach jego wiernych",
          correct: false,
        },
        { text: "Robić to co głos Innosa przekaże", correct: false },
      ],
    },
    {
      id: 9,
      question: "Kto dowodził Pradawnymi i czym była jego broń?",
      answers: [
        { text: "Wieczny Wędrowiec z kosturem", correct: true },
        { text: "Wiekopomny Wędrowiec z berłem", correct: false },
        { text: "Wieczny Wędrowiec z berłem", correct: false },
        { text: "Wiekopomny Wędrowiec z kosturem", correct: false },
      ],
    },
    {
      id: 10,
      question: "Kto zawalił wieżę w Starym Obozie?",
      answers: [
        { text: "Drago", correct: true },
        { text: "Milten", correct: false },
        { text: "Baraback", correct: false },
        { text: "Lester", correct: false },
      ],
    },
  ],
  guilds: [
    {
      easy: [
        {
          id: 1,
          guildName: "Kopacz",
          quoteText:
            "'Te przeklęte stwory przychodzą chyba wprost z samego królestwa Beliara!'",
          desc: "Oj słabiutko. Czy na pewno kiedyś grał_ś w Gothica? Czy po prostu strzelał_ś odpowiedzi? Bez względu na to czy nie pamiętasz co się tam działo, czy pierwszy raz słyszysz o tej serii, zagraj w nią. Jest świetną przygodą, do której wielu weteranów wraca co jakiś czas, a to o czymś świadczy.",
        },
        {
          id: 2,
          guildName: "Obywatel",
          quoteText:
            "'Słuchajcie, słuchajcie, mieszkańcy Khorinis! Na rozkaz wielmożnego Lorda Hagena, ogłasza się, co następuje!'",
          desc: "No, zawsze to lepiej niż bycie kopaczem. Choć mógł_byś się bardziej popisać. Polecam odświeżyć sobie tą genialną produkcję. Chyba, że miał_ś takie szczęście, że udało Ci sie tyle punktów zebrać przez przypadek. Tak czy inaczej, zagraj w Gothicka.",
        },
        {
          id: 3,
          guildName: "Strażnik miejski",
          quote: "Hej Ty, zaczekaj! Chyba gdzieś Cię już widziałem.",
          longDesc:
            "Ho ho ho. A kogo my tu mamy? Udało Ci się. Przy takim wyniku to już coś mówi o Tobie. Znaczy się, to dopiero łatwy poziom. Spróbuj może na średnim/trudnym. Tam dopiero przekonamy się, czy wiesz coś o tej serii więcej.",
        },
      ],
      medium: [
        {
          guildName: "Najemnik",
          quote: "Hej, ja Cię znam!  Byłaś w Kolonii!",
          longDesc:
            "Zawsze to lepiej niż gdybyś zdobył_a tyle punktów na poziomie łatwym. Domyślam się, że znasz podstawy gry, ale polecam Ci ją powtórzyć.",
        },
        {
          guildName: "Mag",
          quote: "Chwała Innosowi",
          longDesc:
            "Oh boy, ciekawe, którym magiem jesteś. Bez względu na to. Mag? Z takim wynikiem? Lepiej zagraj jeszcze raz, tak dla przypomnienia.",
        },
        {
          guildName: "Paladyn",
          quote: "Będę się musiał rozmówić z tymi obwiesiami!",
          longDesc:
            "Brawo, mając taką wiedze mógłbyś już popisywać się przed znajomymi. Został Ci ostani poziom trudności. Tam udowodnisz, czego jesteś wart.",
        },
      ],
      hard: [
        {
          guildName: "Magnat",
          quote: "A dlaczego myślisz, że potrzebuję twoich usług?",
          longDesc:
            "Magnat. Brawo. Daleko zaszedłeś. Choć... serio? Aż tak trudne pytania masz? Weź się postaraj lub zagraj znów w Gothica.",
        },
        {
          guildName: "Bezimienny",
          quote: "Wszystko będzie w porządku.",
          longDesc:
            "Bohater Myrtany, Khorinis, Pogromca Śniącego i Smoka Ożywieńca. Czegóż to tyś nie dokonał. Aplauz dla Ciebie *klask klask*. Ciekawe tylko kto jest mistrzem w tą grę...",
        },
        {
          guildName: "Wrzód",
          quote: "Hej kolego!",
          longDesc:
            "A więc dotarł_ś tutaj. Nie spodziewałem się tego. Ale gratulacje. Stanąłeś na równi z bogiem tej gry, stworzycielem świata i zabawiaczem owiec. Czapki z głów.",
        },
      ],
    },
  ],
};
